...
namespace Klyte.TouchThis.i18n
{
    internal class TTTLocaleUtils
    {
        ...
        public static string[] getLanguageIndex()
        {
            ...
            saida.m_buffer[0] = Locale.Get("TTT_GAME_DEFAULT_LANGUAGE");
            for (int i = 0; i < locales.Length; i++)
            {
                saida.m_buffer[i + 1] = Locale.Get("TTT_LANG", locales[i]);
            }
        ...
        public static void loadLocale(string localeId, bool force, string prefix = "TTT_", string packagePrefix = "Klyte.TouchThis")
        {
            if (force)
            {
        ...
        {
            string load = TTTResourceLoader.instance.loadResourceString("UI.i18n." + localeId + ".properties");
            if (load == null)
            {
                TTTUtils.doErrorLog("FILE " + "UI.i18n." + localeId + ".properties" + " NOT LOADED!!!!");
                load = TTTResourceLoader.instance.loadResourceString("UI.i18n.en.properties");
                if (load == null)
                {
                    TTTUtils.doErrorLog("LOCALE NOT LOADED!!!!");
                    return;
                }
                localeId = "en";
            }
            var locale = TTTUtils.GetPrivateField<Locale>(LocaleManager.instance, "m_Locale");
            Locale.Key k;
